...
jQuery(document).ready(function ($) {
	...
	if (ai1wmde_settings.token) {
		$.ajax({
			...
			details += '</strong>';
			$('#ai1wmde-dropbox-details').html(details);
			$('#ai1wmde-dropbox-space').html('Dropbox space <strong>' + result.used + '</strong> of <strong>' + result.total + '</strong>');

			// Progress bar
			if (result.progress > 100) {
				$('#ai1wmde-dropbox-progress-bar').width(result.progress + '%').text(result.progress + '%').addClass('ai1wmde-dropbox-progress-bar-error');
			} else {
				$('#ai1wmde-dropbox-progress-bar').width(result.progress + '%').text(result.progress + '%').addClass('ai1wmde-dropbox-progress-bar-default');
			}
		});
	}
});
