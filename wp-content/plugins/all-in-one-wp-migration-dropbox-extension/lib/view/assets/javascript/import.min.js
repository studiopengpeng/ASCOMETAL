...
Vue.filter('icon', function (value) {
	...
	} else if (value === 'page_white_compressed') {
...
var browse_folder = function (path) {
	v.files = false;

	// Get files and directories
	$.ajax(
		{
			url: ai1wmde_import.ajax.metadata_url,
			data: { 'path': path },
			dataType: 'json',
			type: 'GET'
		}
...
var browse_file = function (file) {
	...
	setTimeout(function () {
		v.selected_file = filename;
		v.selected_file_name = filename;
	}, 1);
};
...
var browse_unknown_file = function (file) {
	...
	alert('We are sorry but ' + filename + ' cannot be imported.');
};
			...
			v = new Vue({
				...
				methods: {
					browse: function (item) {
						...
						} else if ($.inArray(item.path.split('.').pop(), ['wpress', 'bin']) > -1) {
							browse_file(item);
						} else {
					...
					import: function () {
						...
						var archive = item.path.split('/').pop();

						// Set global params
						model.setParams([
							{name: 'storage', value: storage},
							{name: 'archive', value: archive},
							{name: 'filePath', value: item.path},
